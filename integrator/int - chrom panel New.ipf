...
Function DisplayNextChrom( chromNumber )
	...
	if ( (chromNumLast - chromNum) > 1 )
		next = NextChromInAS(currCh, chromNum, 0 )
	elseif  ( (chromNumLast - chromNum) < -1 )
		next = NextChromInAS(currCh, chromNum, 0 )
	elseif ( chromNumLast > chromNum )
		next = NextChromInAS(currCh, chromNumLast, -1 )
	else
		next = NextChromInAS(currCh, chromNumLast, 1 )
	endif
		
	if ( next == chromNumLast )
		beep
	endif
...
function ResRespPlot( mol )
	...
	AppendToGraph $AorH vs AS
	AppendToGraph /t $AorH vs tsecs_1904
	...
	ModifyGraph zColor[1]={selpos,*,*,CyanMagenta,0}
	ModifyGraph mode[4]=2,lsize[4]=0
	Label bottom "Chrom Number"
	Label top "Time"
	Label left lftlabel
	SetAxis/A/N=1 left
