...
// this approach is incorrect. The monthly mean variablilty decreases with
// more samples. As n increases the sdev approaches 0.
// this is how the MC_Simulation code below works as of 20200806
function nSampleMonthlyMeans(n)
	variable n
...
// As n increases this method approoaches a fixed number.
function RepeatedAnnualMeans(n)
	variable n
	...
	for(year=0; year<n; year+=1)
		for(month=0; month<12; month+=1)
			months[month] = gnoise(1)
		endfor
		years[year] = mean(months)
	endfor
...
function MC_Simulation([sims, frac, type, loc, insts, mol, prefix])
	...
	Prompt type, "Which simulation", popup, "Remove Data;Perturb Data;Both;Adjust Lats;Perturb data and sites"
	...
	for(i=0; i<NumSimulations; i+=1)
		...
		elseif (type == 5)
			PerturbDatabySD(insts, mol)
			sitesused = SelectSiteSubset()
			nte = "preturb data and sites"
		else
			abort "wronge type"
		endif
		...
		if ((mod(i,100) == 0) && (i>0))
			DisplayMC_Matrix(prefix=prefix, mol=mol, loc=loc, type=type, grth=0)
	...
	MCsim = 0
	sitesused = AllBKsitesMSD	// set to all MSD
...
//		KillDataFolder /Z $bak
		...
		// need to remake interpolation waves
		// probably not nessicary. very slow
		//SetDataFolder root:simulation:
		//string fx = "fx_*_" + inst + "_*_" + mol + "_*"
		//bat("Killwaves /Z @", fx)
		//SetDataFolder root:
...
function PerturbDatabySD(insts, mol)
	string insts, mol
	...
	for(i=0; i<ItemsInList(insts); i+=1)
		...
		for(s=0; s<ItemsInList(Allsites); s+=1)
			...
			// if wave exists add some noise to existing data
			if (exists(wv) == 1 )
				wave MR = $wv
				wave SD = $wvsd
				MR = MR+gnoise(SD)
				...
				// need to remake interpolation waves
				// not nessicary. only very small differences
				//SetDataFolder root:simulation:
				//string fx = "fx_*_" + inst + "_" + site + "_" + mol + "_*"
				//bat("Killwaves /Z @", fx)
				//SetDataFolder $"root:"+fld
			endif
...
function /T SelectSiteSubset()
	
	Wave /T sitecombos = root:simulation:site_combs
	variable idx
	string sites
	do
		idx = round(abs(enoise(numpnts(sitecombos))))
		sites = sitecombos[idx]
	while (FindListItem("thd", sites) >= 0)
	return sites
...
function MC_MatrixAvg_Row(MC)
	...
	for (i=0; i<rows; i+=1)
		//MatrixOp /FREE/o slice = row(MC, i)^t
		//WaveStats /q slice
		WaveStats /Q/RMD=[i,i][] MC		// RMD is for multidimensional
		MCmean[i] = V_avg
		MCsd[i] = V_sdev
	endfor
...
// mean and sd of each row in matrix
function MC_MatrixMinMax_Row(MC)
	wave MC
	
	variable rows = DimSize(MC,0), cols = DimSize(MC,1), i, j
	String MCminmax = NameOfWave(MC) + "_minmax"
	
	String fldrSav0= GetDataFolder(1)
	SetDataFolder root:simulation:
	
	Make /o /n=(rows,2) $MCminmax/Wave=MCmm
	for (i=0; i<rows; i+=1)
		MatrixOp /FREE/o slice = row(MC, i)^t
		WaveStats /q slice
		MCmm[i][0] = V_min
...
function MC_MatrixAvg_Col(MC, [med])
	...
	for (i=0; i<cols; i+=1)
		MatrixOp /FREE/o slice = col(MC, i)					// found bug ^t was on this line 201007
...
Function Poly4_auto(xx, yy, sd, [d1, d2, v])
	...
	CurveFit/M=2/Q/NTHR=0/W=0 poly 4, yy[p1,p2] /X=xx /W=sd /I=1
	Wave coef = W_coef
	Duplicate /O coef, Seas_coef
end
...
Function MonteCarlo_fit(xx, yy, sd, [iters,plt])
	...
	// don't fit to points that are 3 sigma outliers (added 190628)
	Duplicate /FREE yy, yy_org
	Wavestats /Q yy
	yy = SelectNumber(yy>V_avg+V_sdev*3, yy, NaN)
	yy = SelectNumber(yy<V_avg-V_sdev*3, yy, NaN)
...
function global_mean_site_combos(mol, [instruments, recalc])
	string mol
	string instruments
	variable recalc

	if (ParamIsDefault(instruments))
		instruments = "MSD"
	endif
	if (ParamIsDefault(recalc))
		recalc = 0
	endif
	
	SetDataFolder root:
	SVAR Gsitesused = root:S_sitesused
	wave /t site_combs = root:simulation:site_combs
	variable i
	
	wave /Z GM = $"root:global:HATS_Global_"+mol
	wave /Z GMx = $"root:global:HATS_"+mol+"_date"
	wave /Z GMsd = $"root:global:HATS_Global_"+mol+"_sd"
	string GMmat = "root:simulation:GM_"+mol
	
	// step through all site combinations in the site_combs wave
	if (!WaveExists($GMmat) || (recalc == 1))
		make /o/n=(0,numpnts(site_combs)) $GMmat/WAVE=GMmatrix
			
		for(i=0; i<numpnts(site_combs); i+=1)
			Gsitesused = site_combs[i]
			Global_Mean(insts=instruments, prefix="HATS", mol=mol, plot=1, offsets=1)
			if (i==0)
				Redimension/N=(numpnts(GM),-1) GMmatrix
			endif
			GMmatrix[][i] = GM[p]
		endfor
	else
		wave /Z GMmatrix=$GMmat
	endif
	
	MC_MatrixAvg_Row($GMmat)
	MC_MatrixMinMax_Row($GMmat)
	wave /Z GMmatavg = $"root:simulation:GM_"+mol+"_mean"
	wave /Z GMmatsd = $"root:simulation:GM_"+mol+"_sd"
	wave /Z GMmatminmax = $"root:simulation:GM_"+mol+"_minmax"

	// global mean
	string win = mol + "_Global_Mean_Lat_Combos"
	DoWindow /K $win
	Display /K=1/W=(572,196,1405,704)/K=1
	DoWindow /C $win
	AppendToGraph GMmatminmax[][0] vs GMx
	AppendToGraph GMmatminmax[][1] vs GMx
	ModifyGraph lstyle=1,rgb=(48059,48059,48059)
	ModifyGraph mode($NameofWave(GMmatminmax))=7,hbFill($NameofWave(GMmatminmax))=5,toMode($NameofWave(GMmatminmax))=1,rgb($NameofWave(GMmatminmax))=(26205,52428,1)
	AppendToGraph GMmatavg vs GMx
	ModifyGraph rgb($Nameofwave(GMmatavg))=(0,0,0);
	ErrorBars $Nameofwave(GMmatavg) Y,wave=(GMmatsd,GMmatsd)

	Gsitesused = AllBKsitesMSD
	Gsitesused = Allsites
	//Gsitesused = "alt;brw;sum;thd;nwr;kum;mlo;smo;cgo;psa;spo;"
	Global_Mean(insts=instruments, prefix="HATS", mol=mol, plot=1, offsets=1)
	AppendToGraph GM vs GMx
	ErrorBars $NameofWave(GM) Y,wave=(GMsd,GMsd)
	ModifyGraph rgb($Nameofwave(GM))=(65000,10,10)
	ModifyGraph lsize($NameOfWave(GMmatavg))=3,lsize($NameofWave(GM))=3
	Legend/C/N=text0/J "\\s("+Nameofwave(GMmatavg) + ") Mean of site combinations\r\\s("+Nameofwave(GM)+") Global mean of all sites\r\\s("+Nameofwave(GMmatminmax)+") Max to min range"
	Label left "\\f01" + FullMolName(mol)
	Label bottom "Date"
	
	// differences
	make /o/n=(numpnts(GM)) $"root:simulation:GM_"+mol+"_mean_diff"/WAVE=diff, $"root:simulation:GM_"+mol+"_sd_diff"/WAVE=diffsd
	diff = (GM-GMmatavg)/GM*100
	diffsd = (GMsd-GMmatsd)/GMsd*100
	
	//Edit GMx, GM, GMsd, GMmatavg, GMmatsd, diff, diffsd
	
	win = mol + "_diff_GM"
	DoWindow /K $win
	Display /K=1/W=(10,183,571,493) diff vs GMx
	DoWindow /C $win
	Label left "% difference in means"
	Label bottom "Date"
	
	win = mol + "_diffsd_GM"
	DoWindow /K $win
	Display /K=1/W=(12,522,571,842) diffsd vs GMx
	DoWindow /C $win
	Label left "% difference in std"
	Label bottom "Date"
end
