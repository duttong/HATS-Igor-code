...
menu "UCATS_Macros"
	...
	Submenu "CHANNEL 1<B"
		...
		" Pressure ECDs", EngPlot1(pres_ECD2, "Pressure",0)
	...
	Submenu "Ozone<B"
		...
		" Ozone concentration", O3Plot1(o3best, "Ozone (ppm)"); appendtograph o3xbest vs timewo3x; legend
	...
	"-"
		"Make P-ext Synched to OZ<B", TimeMatchData(timewo3,timewgc,pres_extern,o3_pres_extern, 5)
		"Make P-ext Synched to GC<B", TimeMatchData(tsecs_1904,timewgc,pres_extern,gc_pres_extern, 5)
		"Make O3 Synched to GC<B", TimeMatchData(tsecs_1904,timewo3,o3,o3_gc, 20)
	"-"
		Submenu "Sampling"
	...
	Submenu "Thermistors"
		...
		" Sample loops", EngPlot2(temp_SL1, temp_SL2, "Sample Loops (C)",0)
		" Pump" , EngPlot1(temp_pump, "Pump (C)",-20)
...
Function BeforeFileOpenHook(refNum, fileNameStr, pathNameStr, fileTypeStr, fileCreatorStr, fileKind)
	...
	if (cmpstr(fileNameStr[len-3,len], "dat")==0)
		...
		elseif (cmpstr(fileNameStr[0,2], "o3x") == 0)	// ozone X data file found
			...
			return 1
		elseif (cmpstr(fileNameStr[0,2], "o3_") == 0)	// ozone data file found
			Load_Ozone( pathNameStr, fileNameStr )
			return 1
...
function Load_Eng( pathNameStr, fileNameStr )
	...
	// crud check to see if both eng files are loaded.  If so, create injection waves.
...
// handles cross over to new day
function FixTimeWave(timeW)
	wave timeW
	
	variable inc = numpnts(timeW)
	variable /d t1, t0
...
function  CleanUpBadValues( engfilenum )
	...
	string Omegas = "CH1_ECD;CH1_Col;CH2_ECD;CH2_Col;CH3_ECD;CH3_Col;CH3_Post"
	variable inc, BAD1 = -99.9, BAD2 = -999
	
	if ( engfilenum == 1 )
	elseif ( engfilenum == 2)
		do
			wave /Z Omega = $StringFromList(inc, Omegas)
			Omega[0] = NaN
...
Function DataRoutines_Ozone()
	...
	// Remove null data points.  These occure when UCATS is on and OzoneX is off
	Wave YY = O3yy
	Wave HH = O3hh
	Wave MN = O3min
	Wave SS = O3ss
...
function SampleTempsInj() 
	...
	DoWindow /k $win
	Display /W=(14,45,737,401) temp_SL1_inj,temp_SL2_inj vs timeWgc_inj
	DoWindow /C $win
	...
	Label bottom "Time (gmt)"
	SetAxis/A/N=1 left
	Legend/N=text0/J/X=6.55/Y=2.56 "\\s(temp_SL1_inj) temp_SL1_inj\r\\s(temp_SL2_inj) temp_SL2_inj"
End
...
function OzonePlots()
	...
	O3Plot1(o3best, "Ozone (ppb)")
	appendtograph o3xbest vs timewo3x
	legend
	
	O3Plot1(o3temp, "Ozone Cell Temp (C)")
	appendtograph o3xtemp vs timewo3x
	legend
	
	O3Plot1(o3pres, "Ozone Cell Pressure (mbar)")
...
function H2OPlots()
	...
	H2OPlot2(tdl_longH2O, tdl_shortH2O,"Water (ppm)")
	appendtograph tdl_best vs timeWh2o
	ModifyGraph rgb(tdl_best)=(3,52428,1)
...
function ThermPlots()
	...
	EngPlot2(temp_SL1, temp_SL2, "Sample Loops (C)",0)
	EngPlot1(temp_pump, "Pump (C)",-10)
	EngPlot1(temp_amb, "Internal temp (C)",0)
	EngPlot2(temp_gasB_N, temp_gasB_C, "External gas bottles (C)",-10)
